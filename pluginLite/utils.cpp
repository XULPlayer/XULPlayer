...
#define MAX_STATIC_BUFFER_SIZE 1<<21

static char* strBuf = NULL;
char* GetStaticBuffer(const char* data, int bytes)
{
	static int offset = 0;
	if(!strBuf) strBuf = (char*)NPN_MemAlloc(MAX_STATIC_BUFFER_SIZE);
	if (!bytes) bytes = strlen(data) + 1;
	if (bytes > MAX_STATIC_BUFFER_SIZE) {
		return 0;	
	}
	offset += bytes; // assuming this is an atomic operation, no mutex is required
	char *ptr;
	if (offset >= MAX_STATIC_BUFFER_SIZE) {
		offset = 0;
		ptr = strBuf;
	} else {
		ptr = strBuf + offset - bytes;
	}
