...
#define SELECT_INTERVAL 500000
static int connect_with_timeout(HTTP_REQUEST* param, int timeout/* in usec*/)
{
	...
	int err;
	struct timeval tv;
	int ret, count = timeout / SELECT_INTERVAL;
	int err_len;
#ifdef WIN32
	unsigned long val;
#endif

	if ((target_host = gethostbyname((const char*)param->hostname)) == NULL) return -1;
...
#ifdef WIN32
	val = 1;
	ioctlsocket(param->sockfd, FIONBIO, &val);
#else
	fcntl(param->sockfd, F_SETFL, fcntl(socket_server_fd, F_GETFL) | O_NONBLOCK);
#endif
	if (connect(param->sockfd, (struct sockaddr*)&server_addr, sizeof(struct sockaddr))==-1 ) {
#ifndef WIN32
	...
	tv.tv_sec = 0;
	tv.tv_usec = SELECT_INTERVAL;
	FD_ZERO(&set);
	FD_SET(param->sockfd, &set);
	// When the connection will be made, we will have a writeable fd
	while((ret = select(param->sockfd + 1, NULL, &set, NULL, &tv)) == 0) {
		if(count <= 0) {
...
#ifndef WIN32
	fcntl(param->sockfd, F_SETFL, fcntl(param->sockfd, F_GETFL) & ~O_NONBLOCK);
#else
	val = 0;
	ioctlsocket(param->sockfd, FIONBIO, &val);
#endif
	...
	err_len = sizeof(int);
	ret =  getsockopt(param->sockfd, SOL_SOCKET, SO_ERROR, (char*)&err, &err_len);
	if(ret < 0) {
		DEBUG("%s\n",strerror(errno));
		return -1;
	}
...
int httpRequest(HTTP_REQUEST* param, const char* url)
{
	...
	do {
		...
		do {
			...
			if (connect_with_timeout(param, 2000000 /*2 sec*/) < 0) {
