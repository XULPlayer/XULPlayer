...
#define AUTH_CHECK_PASSED (authData == getToken())
...
{
	__int64 tk;
	char psz_apppath[MAX_PATH + 1];
	char psz_plugin[MAX_PATH + 1];
	char *p;
	FILETIME myTime;

	GetModuleFileName(NULL, psz_apppath, MAX_PATH);
	p = strrchr(psz_apppath, '\\');
	if (p == NULL) return -1;
	*p = '\0';
	p = strrchr(psz_apppath, '\\');
	if (p == NULL) return -2;
	*p = '\0';
	_snprintf(psz_plugin, MAX_PATH, "%s\\plugins\\npxulplayer.dll", psz_apppath);
	HANDLE hFile = CreateFile(psz_plugin, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
	if (hFile == NULL) {
		return -3;
	}
	GetFileTime(hFile, &myTime, NULL, NULL);
	CloseHandle(hFile);
	tk = myTime.dwHighDateTime + myTime.dwLowDateTime / 100000000;
...
int NP_PreInit(int arg, void* data, int bytes)
{
	authData = arg;
	memcpy(appdata, data, bytes);
	return 0;
}
              ...
              )
{
  ...
  //if (!AUTH_CHECK_PASSED) return NPERR_GENERIC_ERROR;
  if(pFuncs == NULL)
    return NPERR_INVALID_FUNCTABLE_ERROR;
