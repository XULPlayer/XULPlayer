...
if(typeof(XULPLAYER_PLAYLIST_MANAGER_INCLUDE) != "boolean") {
    ...
    function CPlaylistMan(listManId)
    {
		...
		this.path = null;
		this.selIdx = -1;
		this.bRandom = ($e('plModeShuffle').getAttribute('checked') == 'true');
		this.bRepeat = ($e('plModeRepeat').getAttribute('checked')  == 'true');
		this.bRepeatOne = ($e('plModeRepeatOne').getAttribute('checked')  == 'true');
		this.dirty = false;
    }
	...
	CPlaylistMan.prototype.initialize = function () {
		...
		if(listManPath) {
			...
			for(;child;child=DomHelper.getNextNode(child)) {
				...
				alist.enableRepeatOne(this.bRepeatOne);
    ...
    CPlaylistMan.prototype.newList = function (listPath) {
		...
		this.openNativeList(newListPath);
    };
	...
	CPlaylistMan.prototype.enableRepeatOne = function() {
		this.bRepeatOne = ($e('plModeRepeatOne').getAttribute('checked')  == 'true');
		for(var i=0; i<this.lists.length; ++i) {
			this.lists[i].enableRepeatOne(this.bRepeatOne);
		}
	};
	...
	CPlaylistMan.prototype.save = function() {
		...
		var rootNode = DomHelper.getChildNode(this.doc);
		// Get last play list index
		rootNode.setAttribute("lastPlayIndex", this.selIdx);
	...
	CPlaylistMan.prototype.openNativeList = function (listPath, listName) {
		if(!listPath) return;
		...
		if(this.doc) {
			...
			DomHelper.insertNode(this.doc, rootNode, listName, listPath);
			this.dirty = true;
		}
