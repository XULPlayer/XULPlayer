...
{
    $e("videoBox").setAttribute('selectedIndex', page_index);
...
{
	if(playType == PLAY_TYPE_FILE) {
		if(!multiVideoMode) mp.stop();
	} else if(playType == PLAY_TYPE_SWF) {
        swfPlayer.stop();
    }
...
function isPlayerWorking()
{
    if(playType == PLAY_TYPE_SWF) {
        return swfPlayer.isPlaying();
    } else {
        return mp.isPlaying();
    }
}
...
function openDownloader(url)
{
    ...
    window.openDialog('chrome://xulplayer/content/downloader/manager.xul','Download Manager',
                    'chrome, centerscreen, dialog=no, dependent=yes', downloader, url);
}
...
function locateFileItem()
{
	if(plMan && mp) {
		var selFileName = plMan.getSelectedFile();
		mp.shellExecute(getFileDir(selFileName));
	}
}
...
function swfLoadLoop()
{
	if(swfPlayer.plugin.PercentLoaded() < 100) {
		// display loading percent animate
		loadTimerId = setTimeout(swfLoadLoop, 150);
	} else {
...
SwfPlayer.prototype.setPlayFile = function(fileName) {
	...
	this.plugin.setAttribute('loop', "false");
	this.plugin.setAttribute('swliveconnect', "true");
	this.plugin.setAttribute('allowScriptAccess',"always");
...
SwfPlayer.prototype.forward = function() {
    var frameNum = this.getCurFrame();
    frameNum += 12;
    dumpMsg("Goto frame: "+frameNum+"; Total frames: "+this.getTotalFrames());
    this.gotoFrame(frameNum);
};

SwfPlayer.prototype.rewind = function() {
    var frameNum = this.getCurFrame();
    frameNum -= 12;
    this.gotoFrame(frameNum);
};
...
SwfPlayer.prototype.playLoop = function() {
	...
	if(this.isPlaying() && this.file) {
		...
		this.timerId = setTimeout(function(){me.playLoop()}, 150);
	} else {
        clearTimeout(this.timerId);
