...
rd chrome_xxx /s /q
..\webencoder\encrypt\bin\encrypt.exe chrome chrome_xxx

if not exist "chrome_xxx" (
    echo Can't find chrome_xxx directory
    exit /b 1
)

cd chrome_xxx
md ..\release\chrome
7z a -tzip -r -mx=0 -x!.svn ..\release\chrome\%APPNAME%.jar -x!*.zip -x!skin -x!*.manifest -x!*.kpf
pause
rem Packing skins
cd ..\chrome\skin\classic
del ..\Classic.zip
7z a -tzip -r -mx=0 -x!.svn ..\Classic.zip
rem cd ..\pinky
rem del ..\Pinky.zip
